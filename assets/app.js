...
function generateRandomTextMap(rows, cols) {
    ...
    for (let i = 0; i < rows; i++) {
        ...
        // Dört kenarı duvarla yapalım
            if (i === 0 || i === rows - 1 || j === 0 || j === cols - 1 || j === cols - 1) {
                row += 'W';  // Kenarlar duvar
            } else {
                // İç kısmı daha düzenli tutalım, dağınık duvarları azaltalım
                const randomElement = Math.random() < enemyChance ? 'E' : elements[Math.floor(Math.random() * elements.length)];
                row += randomElement;
            }
        }
        map.push(row);
...
// Kullanım örneği
const TextMap = generateRandomTextMap(20, 39);
...
EntityCollision.arcToWall = function (arcX, arcY, arcRadius, wallX, wallY, wallSize) {
    ...
    if (distX > wallSize / 2 + arcRadius) {
        return false;
    }
    if (distY > wallSize / 2 + arcRadius) {
        return false;
    }
...
var Enemy = function (x, y) {
    ...
    this.update = function () {
        if (this.sleep || this.dead || gamestart.style.display === "") return;
...
var Camera = function () {
    ...
    this.update = function () {
        ...
        if (frames >= 15) {
            ...
            var screen = {
                x: player.x - this.offsetX - context.canvas.width / 2 - blockSize,
                y: player.y - this.offsetY - context.canvas.height / 2 - blockSize,
                width: context.canvas.width + blockSize * 2,
                height: context.canvas.height + blockSize * 2,
            };
...
var enemyLength = mapProcessor.getEnemyPositions().length;
